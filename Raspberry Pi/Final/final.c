...
int main ()
{
  ...
  pinMode (15, OUTPUT); // CS_PANTALLA


	//char buff[128];
    char *portname = "/dev/ttyACM0";

	//FILE *fd = fopen (portname, O_RDWR | O_NOCTTY | O_SYNC);
   ...
   delay(1);
	  ...
	  for(it = 0; it < 2; ++it) {
		...
		if (tactil[0] == 0 || tactil[1] == 0) {
	  	...
	  	fprintf(fd, "CoordX: %d, CoordY: %d\n",tactil[0], tactil[1]);
		printf("CoordX: %d, CoordY: %d\n",tactil[0], tactil[1]);

    char clean[100]	= "                             ";
	pintaString(10,10, clean, strlen(clean));
    pintaString(10,30, clean, strlen(clean));


	//fflush(fdr);

	char s[100];
	sprintf(s, "CoordX: %d, CoordY: %d\n",tactil[0], tactil[1]);
	pintaString(10,10, s, strlen(s));
	sprintf(s,"X: %d, Y: %d, Z: %d \n", readVal[0], readVal[1], readVal[2]);
	pintaString(10,30, s, strlen(s));
	
	
    char *line = NULL;
    size_t len = 0;
	ssize_t read;
	if ((read = getline(&line, &len, fdr)) != -1) {
		pintaString(10,50, clean, strlen(clean));
		pintaString(10,50, line, read);
	}

	if ((read = getline(&line, &len, fdr)) != -1) {
		pintaString(10,70, clean, strlen(clean));
		pintaString(10,70, line, read);
	}
